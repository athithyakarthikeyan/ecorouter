"""
Chennai EcoRouter - Flask Backend v6
Fixes vs uploaded v5:
  1. routeRepresentation=polyline added to TomTom params (fixes 400 error + straight lines)
  2. Geocode uses Chennai bbox rect filter (fixes missing suburbs like Medavakkam)
  3. eco_is_fast flag: only true when no alternative exists AND CO2 identical
  4. API keys use os.getenv with fallback (no crash if .env missing)
"""
...
# Chennai + suburbs bbox for geocoding: minLon,minLat,maxLon,maxLat
...
def load_graph():
    if GRAPH_CACHE.exists():
        ...
        try:
            # Maximum coverage: entire Greater Chennai Metropolitan Area
            # North: Tiruvallur (13.55), South: Chengalpattu (12.60)
            # West: Sriperumbudur/Koyembedu (79.85), East: coast (80.38)
            # Covers: Koyembedu, Ambattur, Avadi, Tambaram, OMR, GST Road,
            #         Maraimalai Nagar, Kelambakkam, Porur, Poonamallee etc.
    ...
    # Apply Chennai-realistic speed correction:
    # OSMnx defaults are too optimistic. Indian urban roads average 20-35 kph
    # due to mixed traffic, signals, and road conditions.
    SPEED_CORRECTION = {
        "motorway":      65,   # NH / expressway
        "trunk":         50,   # major arterials (GST, OMR, ECR)
        "primary":       35,   # main city roads
        "secondary":     25,   # secondary roads
        "tertiary":      20,   # local roads
...
def snap_to_roads(coords) -> list:
    ...
    if len(coords) < 2:
        return coords
    try:
        # TomTom Snap accepts points as a colon-separated lat,lon string
        # Max 100 points per request — sample if needed
...
def path_dist_time(G, nodes, traffic_factor=1.0):
    ...
    for u, v in zip(nodes, nodes[1:]):
        ...
        # Inflate travel time by traffic: at tf=0.5 (half speed), time doubles
...
def api_geocode():
    ...
    try:
        # TomTom Fuzzy Search — much better than Geoapify for Indian localities
        # topLeft/btmRight restrict results to Greater Chennai region
        r = requests.get(
